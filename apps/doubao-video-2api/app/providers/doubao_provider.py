...
class DoubaoProvider(BaseProvider):
    def __init__(self):
        self.credential_manager = CredentialManager.shared()
    ...
    async def initialize(self):
        ...
        await self.playwright_manager.initialize(self.credential_manager.active_credentials)
    ...
    async def _non_stream_completion(self, request_data: Dict[str, Any]) -> JSONResponse:
        ...
        try:
            async with self.credential_manager.acquire() as lease:
                ...
                if settings.DOUBAO_CHAT_QUOTA_COST > 0:
                    await self.credential_manager.consume_quota(
                        lease.index,
                        settings.DOUBAO_CHAT_QUOTA_COST,
            ...
            logger.warning(f"Doubao credential pool is busy: {e}")
            return JSONResponse(
                status_code=429,
                content={"error": {"message": str(e), "type": "rate_limit_error", "code": "credential_pool_busy"}}
            )
    ...
    async def _stream_generator(self, request_data: Dict[str, Any]) -> AsyncGenerator[bytes, None]:
        ...
        try:
            ...
            logger.warning(f"Doubao credential pool is busy: {e}")
            error_chunk = create_chat_completion_chunk(request_id, user_model, str(e), "stop")
            yield create_sse_data(error_chunk)
            yield DONE_CHUNK
        except Exception as e:
