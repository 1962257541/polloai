...
import { PublicProfileScraperService } from "./public-profile-scraper.service";
...
export class MonitorWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    private readonly scraper: PublicProfileScraperService,
    private readonly heartbeat: HeartbeatService,
    private readonly env: EnvService,
  ) {}

  async onModuleInit() {
    const concurrency = this.env.scraperPoolSize;
    this.worker = new Worker(
      ...
      { connection: this.env.redisConnection, concurrency },
    );
    ...
    this.logger.log(`MonitorWorker started (queue=tt-scrape, concurrency=${concurrency})`);
  }
